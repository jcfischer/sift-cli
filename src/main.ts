...
program
  ...
  .option('--hybrid', 'Use hybrid search (30% keyword + 70% semantic embeddings). Requires --topic')
  .action(async (query: string, opts: { limit: string; since?: string; topic?: string; hybrid?: boolean }) => {
    const limit = parseInt(opts.limit, 10);
    try {
      const client = getClient();

      if (opts.hybrid && !opts.topic) {
        console.error('--hybrid requires --topic (backend needs a topic scope for embedding search)');
        process.exit(1);
      }

      const searchOpts: { since?: string; topicId?: number; mode?: 'keyword' | 'hybrid' } = {};
      if (opts.hybrid) searchOpts.mode = 'hybrid';
      ...
      for (const r of results) {
        ...
        if (r.relevance_score != null) console.log(`   📊 relevance: ${(r.relevance_score * 100).toFixed(1)}%`);
